...
export const GetWeather = async({ setWeatherTemp,setPop,dateRange,locationId,locationName}) => {
   const res = await axios.get(`https://opendata.cwa.gov.tw/api/v1/rest/datastore/${locationId}?Authorization=CWB-25DEA212-F3AA-4FDB-B05C-30DEF7E737DC&&LocationName=${locationName}${dateRange}`)
      ...
      const weatherDescriptionData = weatherElement.find(({ElementName})=>ElementName === '天氣預報綜合描述').Time
console.log('weatherDescriptionData',weatherDescriptionData)
      weatherDescriptionData.forEach(({StartTime},index)=>{
         const weatherDescription = weatherDescriptionData[index].ElementValue[0].WeatherDescription;
